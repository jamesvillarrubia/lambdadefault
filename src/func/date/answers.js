...
function correct(c) {
  ...
  let start = moment.utc(cand.start_utc);
  let results = [];
  let chrono = JSON.parse(cand.chrono)[0];
  switch (cand.style) {
    //string matches known bools for dmydmy of start/end
    case '1100000000': //April 8
      results.push(start.format('MMMM Do'));
      break;
    case '1000000000': //8 IGNORE OR DELETE
      break;
    case '1110000000': //April 8, 2016
      results.push(start.format('MMMM D, YYYY'));
      break;
    case '0110000000': //April 2016 
    case '0110001100': //April 2016 to February 2017 OR ALL OF APRIL
      results.push(start.format('MMMM YYYY'));
      break;
    case '0100001000':
    case '0100000000': //August
      results.push(start.format('MMMM'));
      break;
    case '0110001100':
      break;
    case '0010000100': //Year
    case '0011000100': //Decade
      if (chrono.start) {
...
function randDay(date) {
  const alter = Math.floor(Math.random() * 10) + 1;
...
function randYear(date) {
  const alter = Math.floor(Math.random() * 6) + 1;
...
function randDecade(date) {
  const alter = (Math.floor(Math.random() * 5) + 1) * 10;
  if (Math.random() > 0.5) {
    date.add(alter, 'years');
  } else {
    date.subtract(alter, 'years');
...
function styleMatch(cand) {
  ...
  //console.log(cand);
  try {
    const c = JSON.parse(cand.chrono)[0];
    let boolS;
    let boolE;
    //console.log(c);
    if (c.start) {
      //console.log(c.start);
      const boolSDay = c.start.knownValues.hasOwnProperty('day') ? 1 : 0;
      const boolSMonth = c.start.knownValues.hasOwnProperty('month') ? 1 : 0;
      const boolSYear = c.start.knownValues.hasOwnProperty('year') ? 1 : 0;
      const boolSDecade = c.start.knownValues.hasOwnProperty('decade') ? 1 : 0;
      const boolSCentury = c.start.knownValues.hasOwnProperty('century') ? 1 : 0;
      boolS = `${boolSDay}${boolSMonth}${boolSYear}${boolSDecade}${boolSCentury}`;
    } else {
      boolS = '00000';
    }
    if (c.end) {
      const boolEDay = c.end.knownValues.hasOwnProperty('day') ? 1 : 0;
      const boolEMonth = c.end.knownValues.hasOwnProperty('month') ? 1 : 0;
      const boolEYear = c.end.knownValues.hasOwnProperty('year') ? 1 : 0;
      const boolEDecade = c.end.knownValues.hasOwnProperty('decade') ? 1 : 0;
      const boolECentury = c.end.knownValues.hasOwnProperty('century') ? 1 : 0;
